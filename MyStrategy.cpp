...
void MyStrategy::act(const Robot &me, const Rules &rules, const Game &game, Action &action) {
    ...
    if (is_attacker) {
        ...
        for (int i = 1; i < 100; i++) {
            ...
            Vec2 ball_pos = Vec2(game.ball.x, game.ball.y).add(ss);
...
// Найдем время и место, в котором мяч пересечет линию ворот
        double t = (target_pos.getY() - game.ball.y) / game.ball.velocity_y;
...
// Установка нужных полей для желаемого действия
    Vec2 target_velocity = Vec2(target_pos.getX() - me.x, target_pos.getY() - me.y).mul(ROBOT_MAX_GROUND_SPEED);
    action = Action();
    action.target_velocity_x = target_velocity.getX();
    action.target_velocity_y = 0.0;
    action.target_velocity_z = target_velocity.getY();
