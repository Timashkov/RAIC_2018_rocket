...
Dan Simulation::dan_to_arena_quarter(Vec3 point){    // Ground
    Dan dan = dan_to_plane(point, Vec3(0, 0, 0), Vec3(0, 1, 0));
    ...
    // Side z (goal)
    dan = min(dan, dan_to_plane(point, Vec3(0, 0, (arena.depth / 2) + arena.goal_depth), Vec3(0, 0, -1)));
    
    // Side z
    Vec3 v = Vec3(point.getX(), point.getY(),0) - (Vec3((arena.goal_width / 2) - arena.goal_top_radius,arena.goal_height - arena.goal_top_radius,0));
    ...
    if (point.getZ() >= (arena.depth / 2) + arena.goal_side_radius){
        // x
        dan = min(dan, dan_to_plane(point, Vec3(arena.goal_width / 2, 0, 0), Vec3(-1, 0, 0)));
        // y
    ...
    if (point.getZ() > (arena.depth / 2) + arena.goal_depth - arena.bottom_radius){
        dan = min(dan, dan_to_sphere_inner(point,
                                           Vec3(clamp(point.getX(), arena.bottom_radius - (arena.goal_width / 2),(arena.goal_width / 2)- arena.bottom_radius),
                                                clamp(point.getY(),
                                                      arena.bottom_radius,
    ...
    if (point.getX() > (arena.width / 2) - arena.corner_radius
        and point.getZ() > (arena.depth / 2) - arena.corner_radius){
    ...
    if (point.getZ() < (arena.depth / 2) + arena.goal_side_radius){
        ...
        Vec3 v = Vec3(point.getX(), point.getY(),0) - o1;
        if (v.getX() > 0 and v.getY() > 0){
            Vec3 o = o1 + v.normalized() * (arena.goal_top_radius + arena.goal_side_radius);
            dan = min(dan, dan_to_sphere_outer(
                                               point,
                                               Vec3(o.getX(), o.getY(), (arena.depth / 2) + arena.goal_side_radius),
                                               arena.goal_side_radius));
    ...
    if (point.getY() < arena.bottom_radius){
        ...
        if (point.getZ() > (arena.depth / 2) - arena.bottom_radius
            and point.getX() >= (arena.goal_width / 2) + arena.goal_side_radius){
            dan = min(dan, dan_to_sphere_inner(
                                               point,
                                               Vec3(
                                                    point.getX(),
        ...
        Vec3 v = Vec3(point.getX(), point.getZ(),0) - o1;
        if (v.getX() < 0 && v.getY() < 0 && v.len() < arena.goal_side_radius + arena.bottom_radius){
        ...
        // Side x (goal)
        if (point.getZ() >= (arena.depth / 2) + arena.goal_side_radius
            and point.getX() > (arena.goal_width / 2) - arena.bottom_radius){
            dan = min(dan, dan_to_sphere_inner(point,
                                               Vec3(
        ...
        if (point.getX() > (arena.width / 2) - arena.corner_radius
            and point.getZ() > (arena.depth / 2) - arena.corner_radius){
            Vec3 corner_o = Vec3((arena.width / 2) - arena.corner_radius,
                                 (arena.depth / 2) - arena.corner_radius, 0 );
            Vec3 n = Vec3(point.getX(), point.getZ(),0) - corner_o;
            double dist = n.len();
    ...
    if (point.getY() > arena.height - arena.top_radius){
        ...
        if (point.getX() > (arena.width / 2) - arena.corner_radius
            ...
            Vec3 corner_o = Vec3( (arena.width / 2) - arena.corner_radius,
                                 (arena.depth / 2) - arena.corner_radius, 0);
            Vec3 dv = Vec3(point.getX(), point.getZ(),0) - corner_o;
...
Dan Simulation::dan_to_arena(Vec3& point){
    bool negate_x = point.getX() < 0;
    bool negate_z = point.getZ() < 0;
    if (negate_x){
...
void Simulation::collide_entities(Entity& a,Entity& b){
    ...
    if (penetration > 0){
        ...
        if (delta_velocity < 0){
            Vec3 impulse = normal * (1 + random(rules.MIN_HIT_E, rules.MAX_HIT_E)) * delta_velocity;
...
Vec3 Simulation::collide_with_arena(Entity& e){
    ...
    if (penetration > 0){
        ...
        if (velocity < 0){
            e.velocity = e.velocity - danArena.normal * velocity * (1 + e.arena_e);
...
void Simulation::move(Entity& e, double delta_time){
    ...
    e.position.setY(e.position.getY() - rules.GRAVITY * delta_time * delta_time / 2);
...
void Simulation::update(double delta_time){
    ...
    for (Entity robot : robots){
        if (robot.touch){
            ...
            if (target_velocity_change.len() > 0){
                double acceleration = rules.ROBOT_ACCELERATION * max(0, robot.touch_normal.getY());
    ...
    if (abs(ball.position.getZ()) > arena.depth / 2 + ball.radius){
        goal_scored();
    }
...
void Simulation::tick(){
    double delta_time = 1 / rules.TICKS_PER_SECOND;
