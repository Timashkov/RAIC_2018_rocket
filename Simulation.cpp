...
//    int skipcount = 0;
    while (tn->children.size() > 0) {
        cout << "Hit position " << tn->state.ballHitPosition.toString() << " for tick " << tn->state.current_tick << endl;
...
//                skipFurtherAnalyze = false;
    ...
    if (ticks_till_collision != -1 && ticks_till_collision != on_collision_tick-tn->state.current_tick){
        ...
        int target_tick = (on_collision_tick - tn->state.current_tick - ticks_till_collision)/2 + tn->state.current_tick;
