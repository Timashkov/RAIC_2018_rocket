...
//        cout << "Hit position " << hitPosition.toString() << " for tick " << tn->state.current_tick << endl;
        ...
        for (SimulationEntity &se: tn->state.robots) {
            if (se.teammate && se.touch &&
                (((hitPosition - se.position).len() < rules.arena.depth / 2.0 && se.id == goalKeeperId) ||
                 se.id != goalKeeperId)
...
//    cout << "ACTIONS " << tn->state.current_tick << endl;
    if (collision_tick != -1) {
        while (tn->children.size() > 0 && tn->state.current_tick <= collision_tick) {
...
//                cout << " Default Go 2 position " << defaultGoalKeeperPosition.toString() << " parentNode tick: "
//                     << tn->state.current_tick
//                     << endl;
...
//    cout << "ACTIONS DONE" << endl;
    ...
    if (tn->parent != NULL && collision_tick != -1) {
        tn = tn->parent;
...
int Simulation::checkAchievement(SimulationEntity rr1, Vec3 bptarget, Vec3 excludeTarget, int max_attempts) {
    ...
    int step = 0;
    bool collided = false;
    while (!collided && step < max_attempts + 3) {
        for (int j = 0; j < 100; j++) {
            ...
            Vec3 delta_position = rr1.position - bptarget;

            if (delta_position.len() < 3.0) {
                collided = true;
            }
...
Vec3 Simulation::getEscapePosition(Vec3 ball_moment_position) {
    //attacker goes to ball position


    Vec3 escapePosition = ball_moment_position;
    if (ball_moment_position.getX() > 0) {
        escapePosition.setX(escapePosition.getX() - 4);
    } else {
        escapePosition.setX(escapePosition.getX() + 4);
    }
    escapePosition.setZ(escapePosition.getZ() - 4);
